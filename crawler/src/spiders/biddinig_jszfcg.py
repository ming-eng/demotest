...
class BiddingJANGSUZBSpider(scrapy.Spider):
    ...
    def __init__(self):
        ...
        #竞争切磋
        self.jzqsgg_url='http://www.ccgp-jiangsu.gov.cn/ggxx/jzqsgg/index_{}.html'
        #单一来源
        self.dylygg_url='http://www.ccgp-jiangsu.gov.cn/ggxx/dylygg/index_{}.html'
        #询价公告
        self.xjgg_url='http://www.ccgp-jiangsu.gov.cn/ggxx/xjgg/index_{}.html'
        #中标公告
        self.zbgg_url ='http://www.ccgp-jiangsu.gov.cn/ggxx/zbgg/index_{}.html'
        # 成交公告
        self.cgcjgg_url ='http://www.ccgp-jiangsu.gov.cn/ggxx/cgcjgg/index_{}.html'
        ...
        self.tml_logger = init_tml_logger("bidding_jszfcg")
    ...
    def start_requests(self):
        ...
        yield scrapy.Request(url=self.zgysgg_url.format(1),
                             callback=self.parse_bidding_infos_from_page,
                             headers=self.headers,
                             dont_filter=True,
                             meta={'key': '资格审查公告',
                                   'source_url':self.zgysgg_url})

        yield scrapy.Request(url=self.yqzbgg_url.format(1),
                             callback=self.parse_bidding_infos_from_page,
                             headers=self.headers,
                             dont_filter=True,
                             meta={'key': '邀请招标公告',
                                   'source_url':self.yqzbgg_url})

        yield scrapy.Request(url=self.jztbgg_url.format(1),
                             callback=self.parse_bidding_infos_from_page,
                             headers=self.headers,
                             dont_filter=True,
                             meta={'key': '竞争谈判',
                                   'source_url':self.jztbgg_url})

        yield scrapy.Request(url=self.jzqsgg_url.format(1),
                             callback=self.parse_bidding_infos_from_page,
                             headers=self.headers,
                             dont_filter=True,
                             meta={'key': '竞争切磋',
                                   'source_url':self.jzqsgg_url})

        yield scrapy.Request(url=self.dylygg_url.format(1),
                             callback=self.parse_bidding_infos_from_page,
                             headers=self.headers,
                             dont_filter=True,
                             meta={'key': '单一来源',
                                   'source_url':self.dylygg_url})

        yield scrapy.Request(url=self.xjgg_url.format(1),
                             callback=self.parse_bidding_infos_from_page,
                             headers=self.headers,
                             dont_filter=True,
                             meta={'key': '询价公告',
                                   'source_url':self.xjgg_url})

        yield scrapy.Request(url= self.zbgg_url.format(1),
                             callback=self.parse_bidding_infos_from_page,
                             headers=self.headers,
                             dont_filter=True,
                             meta={'key': '中标公告',
                                   'source_url':self.zbgg_url})

        yield scrapy.Request(url= self.cgcjgg_url.format(1),
                             callback=self.parse_bidding_infos_from_page,
                             headers=self.headers,
                             dont_filter=True,
                             meta={'key': '成交公告',
                                   'source_url':self.cgcjgg_url})
