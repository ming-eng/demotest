...
class BiddingJANGSUZBSpider(scrapy.Spider):
    ...
    def __init__(self):
        ...
        self.max_page = 50
        # 招标公告列表页
        self.bidding_index_url = 'http://zbtb.gxi.gov.cn:9000/xxfbcms/category/bulletinList.html?searchDate=1995-07-25&dates=300&word=&categoryId=88&industryName=&area=&status=&publishMedia=&sourceInfo=&showStatus=1&page={}'
    ...
    def _get_num_of_pages(self, response):
        try:
            pages = response.css('#pageTotal::attr(value)').extract_first()
            return pages
        except:
            pages = self.max_page
            return pages
    ...
    def parse_bidding_infos_from_page(self, response):
        ...
        print(nb_pages)
        nb_pages = 2
        self.tml_logger.info("【bidding_gxzbtb】输入关键词{}获得了{}个网页".format('key', nb_pages))
        for page in range(1, int(nb_pages) + 1):
            url = self.bidding_index_url.format(page)
            self.tml_logger.info("【bidding_gxzbtb】关键词{}在第{}页开始请求url: {}".format(key, page, url))
            try:
                ...
                self.tml_logger.error(
                    "【bidding_gxzbtb】关键词{}在第{}页请求url：{}时失败，失败原因{}".format('key', page, url, str(e)))
                continue
    ...
    def extract_bidding_infos_from_page(self, response):
        ...
        for trs in css_text:
            ...
            for index,td in enumerate(tds):
                if index==0:
                    data['href'] = td.xpath('./a/@href').extract_first().replace("javascript:urlOpen('",'').replace("')",'')
                    data['title'] = td.xpath('./a/text()').extract_first().strip()

                if index==4:
                    data['create_time']=td.xpath('./text()').extract_first().strip()
            datas.append(data)

        self.tml_logger.info("【bidding_gxzbtb】关键词{}在第{}页，开始抽取列表页内容".format(key, page))
        for data in datas:
            #判断是否为空字典
            if not bool(data ):
                # 去除时间标签中不含时间元素的标签
                ...
                self.tml_logger.info("【bidding_gxzbtb】关键词{}在第{}页，请求网页详情url：{}".format(key, page, detail_url))
                # 判断时间
    ...
    def get_content(self, response):
        ...
        self.tml_logger.info("【bidding_gxzbtb】关键词{}在第{}页，开始抽取网页：{} 的详情内容".format(key, page, url))
        # time.sleep(self.wait_time)
        self.tml_logger.info("【bidding_gxzbtb】关键词{}在第{}页，网页：{} 的详情内容开始填充item".format(key, page, url))
        item = BiddingItem()
        item['url'] = url
        item['url_time'] = url_time
        item['source'] = '广西壮族自治区招标投标公共服务平台'
        ...
        self.tml_logger.info("【bidding_gxzbtb】关键词{}在第{}页，网页：{} 的详情内容item即将提交".format(key, page, url))
        yield item
